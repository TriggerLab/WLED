...
#define WLED_LONG_REPEATED_ACTION   250 // how often a repeated action (e.g. dimming) is fired on long press on button IDs >0
...
#define WLED_LONG_BRI_STEPS          5 // how much to increase/decrease the brightness with each long press repetition
    ...
    if (isButtonPressed(b)) { // pressed
      ...
      if (now - buttons[b].pressedTime > WLED_LONG_PRESS) { //long press
        if (!buttons[b].longPressed) {
          ...
          if (now - buttons[b].pressedTime >= WLED_LONG_REPEATED_ACTION) {
            longPressAction(b);
            buttons[b].pressedTime = now;
          }
        }
